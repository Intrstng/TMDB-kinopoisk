        ...
        <Box component="section" sx={movieDetailsSx.details}>
            ...
            <Box>
                ...
                <Box sx={movieDetailsSx.filmInfo}>
                    <Typography variant="h4" component="h4" sx={movieDetailsSx.filmYear}>Release year: <Box component="span" sx={movieDetailsSx.filmYearSpan}>{getYearFromDate(filmData.release_date)}</Box></Typography>
                    <Typography variant="h4" component="h4" sx={movieDetailsSx.vote}>{filmData.vote_average.toFixed(1)}</Typography>
                    <Typography variant="h4" component="h4" sx={movieDetailsSx.filmRuntime}>Runtime: <Box component="span" sx={movieDetailsSx.filmRuntimeSpan}>{getFilmRuntime(filmData.runtime)}</Box></Typography>
                </Box>

                <Typography variant="h3" component="h3" sx={movieDetailsSx.genresTitle}>Genres</Typography>
                <Box sx={movieDetailsSx.genresList}>
                    {filmData.genres.map((genre) => (
                        <Typography key={genre.id} variant="h5" component="h5" sx={movieDetailsSx.genreItem}>{genre.name}</Typography>
                    ))}
                </Box>
            </Box>
        </Box>
